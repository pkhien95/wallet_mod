...
import {Alert, Animated, Clipboard, Dimensions, Platform, StyleSheet, Text, View} from 'react-native'
...
export default class HomeScreen extends Component {
	...
	onUnlock = () => {
		...
		MainStore.appState.wallets.map((wallet, index) => {
			MainStore.appState.wallets[index].tokens = [
				{
					title: "IRobot",
					symbol: "IRB",
					balanceToken: 0,
			...
			this.getIRBBalance(wallet.address, index);
		});
		// setTimeout(() => {
		// 	Alert.alert(null, JSON.stringify(MainStore.appState.wallets));
		// }, 3000)
	};
	
	getIRBBalance = (address, index) => {
		//0x46ad5dfb21c255fe887ede3619395161f669dbba
		try {
			const tokenAddress = "0x3161a3a2469469c663d2fd4cdbda29224c8858c1";
			const apiKey = "RAJ3X4EXTPVZWCJVWNSRKJD94QYFTWGSJN";
			fetch(`https://api.etherscan.io/api?module=account&action=tokenbalance&contractaddress=${tokenAddress}&address=${address}&tag=latest&apikey=${apiKey}`)
				.then(res => {
					return res.json();
				})
				.then(json => {
					if(json.result) {
						MainStore.appState.wallets[index].balance = json.result;
						// MainStore.appState.wallets[index].totalBalanceDollar = json.result;
						MainStore.appState.wallets[index].tokens[0].balanceToken = json.result;
						MainStore.appState.wallets[index].tokens[0].balanceInDollar = json.result;
						if(index === MainStore.appState.wallets.length - 1) {
							this.setState(prevState => ({...prevState, updatedWalletList: Array.from(MainStore.appState.wallets)}))
						}
					}
				});
		} catch (error) {
	...
	render() {
		...
		this.cards = this.state.updatedWalletList.length > 0 ? this.state.updatedWalletList : this.wallets;
			...
			<View style={styles.container}>
				...
				<View style={{height: heightCarousel}}>
					<Carousel
						...
						ref={(c) => {
							MainStore.appState.homeCarousel = c
						}}
						data={this.cards}
						layout="default"
